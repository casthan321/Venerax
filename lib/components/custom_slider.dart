...
class _CustomSliderState extends State<CustomSlider> {
  ...
  double valueFromPosition(double dx, double width) {
    dx = dx.clamp(0.0, width).toDouble();
    if (widget.reversed) {
      dx = width - dx;
    }
    final gap = width / widget.divisions;
    final gapValue = (widget.max - widget.min) / widget.divisions;
    return ((dx / gap).round() * gapValue + widget.min)
        .clamp(widget.min, widget.max)
        .toDouble();
  }

  void updateValue(double dx, double width) {
    final nextValue = valueFromPosition(dx, width);
          ...
          ? LayoutBuilder(
              builder: (context, constraints) => MouseRegion(
                ...
                child: GestureDetector(
                  ...
                  onTapUp: (details) {
                    updateValue(details.localPosition.dx, constraints.maxWidth);
                    commitValue();
                  },
                  ...
                  onHorizontalDragEnd: (_) => commitValue(),
                  onHorizontalDragCancel: cancelValue,
                  child: SizedBox(
                    height: 24,
                    child: Center(
                      child: SizedBox(
