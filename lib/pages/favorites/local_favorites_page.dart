...
class _LocalFavoritesPageState extends State<_LocalFavoritesPage> {
  ...
  late final CoalescingAsyncRunner _asyncRefresh;

  bool get isLoading => _asyncRefresh.isRunning;
  ...
  void updateSearchResult() {
    setState(_rebuildSearchResults);
  }

  void _applyComics(List<FavoriteItem> value) {
    comics = value;
    if (searchMode) {
  ...
  Future<void> _loadComicsAsync() async {
    final value =
        await (isAllFolder
                ? manager.getAllComicsAsync()
                : manager.getFolderComicsAsync(widget.folder))
  ...
  void updateComics() {
    ...
    if (isAllFolder) {
      ...
      if (folderComics < _asyncDataFetchLimit) {
        final value = manager.getFolderComics(widget.folder);
        setState(() {
          _applyComics(value);
        });
      } else {
  ...
  void initState() {
    ...
    comics = [];
    _asyncRefresh = CoalescingAsyncRunner(
      operation: _loadComicsAsync,
      onError: (error, stackTrace) {
        Log.error("Load Local Favorites", error.toString(), stackTrace);
      },
      onRunningChanged: (_) {
        if (mounted) {
          setState(() {});
