...
Future<void> openComicFolder(LocalComic comic) async {
  try {
    ...
    final route = resolveFolderOpenRoute(
    ...
    switch (route) {
      case FolderOpenRoute.windowsExplorer:
        await Process.run('explorer', [folderPath]);
        return;
      case FolderOpenRoute.macFinder:
        await Process.run('open', [folderPath]);
        return;
      case FolderOpenRoute.linuxFileManager:
        ...
        return;
      case FolderOpenRoute.androidDocuments:
        const methodChannel = MethodChannel('venera/method_channel');
        final opened =
            await methodChannel.invokeMethod<bool>('openFolder', folderPath) ??
            false;
        if (!opened) {
          ...
          if (App.rootContext.mounted) {
            App.rootContext.showMessage(
              message:
                  'Folder could not be opened. Path copied to clipboard.'.tl,
            );
          }
        }
        return;
      case FolderOpenRoute.mobileUrlLauncher:
        await launchUrlString('file://$folderPath');
        return;
    }
  } catch (e, s) {
    ...
    if (App.rootContext.mounted) {
      if (App.isAndroid) {
        await Clipboard.setData(ClipboardData(text: comic.baseDir));
        App.rootContext.showMessage(
          message: 'Folder could not be opened. Path copied to clipboard.'.tl,
        );
      } else {
