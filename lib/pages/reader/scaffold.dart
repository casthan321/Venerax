...
class _ReaderScaffoldState extends State<_ReaderScaffold> {
  ...
  void addImageFavorite() async {
    try {
      if (!supportsImageFavorites(
        isImportedLocalComic: context.reader.type == ComicType.local,
      )) {
        showToast(
  ...
  Widget buildSlider() {
    ...
    return CustomSlider(
      ...
      onChanged: (i) {
        // Progress seeking can emit a new target before a long-distance
        // continuous-mode animation has mounted. Jumping directly avoids
        // leaving the reader locked behind an unfinished animation future.
        context.reader.toPage(i.toInt(), animate: false);
      },
    );
  }
