...
class JsEngine with _JSEngineApi, JsUiApi, Init {
  ...
  static Future<void> reset() {
    final activeReset = _resetOperation;
    if (activeReset != null) return activeReset;

    late final Future<void> operation;
    operation = _resetOnce().whenComplete(() {
      if (identical(_resetOperation, operation)) {
  ...
  static Future<void> _resetOnce() async {
        ...
        switch (method) {
          case "log":
            ...
            Log.addLog(
              ...
              summarizeNetworkPayload(message["title"], maxCharacters: 128),
  ...
  Future<Map<String, dynamic>> _http(Map<String, dynamic> req) async {
    ...
    try {
      ...
      var dio = _dio;
      final requestedHttpClient = removeHeaderCaseInsensitive(
        headers,
        'http_client',
      );
      if (requestedHttpClient == "dart:io") {
        dio = Dio(
          BaseOptions(
            responseType: ResponseType.plain,
            validateStatus: (status) => true,
        ...
        dio.httpClientAdapter = IOHttpClientAdapter(
          createHttpClient: () {
            ...
            configureHttpClientProxy(client, proxy);
            return client;
          },
        );
...
mixin class _JSEngineApi {
  ...
  void clearCookies(List<String> domains) {
