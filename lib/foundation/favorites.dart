...
class LocalFavoritesManager with ChangeNotifier {
  ...
  void initCounts() {
    ...
    loadFavoriteHashesInIsolate(folderNames, _databasePath).then((value) {
      _hashedIds = value;
      notifyListeners();
    });
  }
  ...
  @visibleForTesting
  static Future<Map<int, int>> loadFavoriteHashesInIsolate(
    List<String> folders,
    String databasePath, {
  ...
  Future<List<FavoriteItem>> getFolderComicsAsync(String folder) {
    return _getFolderComicsAsync(folder, _databasePath);
  }
  ...
  Future<List<FavoriteItem>> getAllComicsAsync() {
    return _getAllComicsAsync(folderNames, _databasePath);
  }
